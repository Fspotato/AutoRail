...
def moveTo(x=None, y=None, duration=0.0, tween=None, logScreenshot=False, _pause=True, relative=False):
    if not relative:
        ...
        if(startx == x and starty == y) : return

        width, height = size()
        steps = [(x, y)]

        if duration > MINIMUM_DURATION:
            nums_steps = max(width, height)
        ...
        for tweenX, tweenY in steps:
            ...
            extra = ctypes.c_ulong(0)
            ii_ = Input_I()
            ii_.mi = MouseInput(tweenX, tweenY, 0, (MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE), 0, ctypes.pointer(extra))
...
def humanLike_getPointOnLine(x1, y1, x2, y2, n):
    xOffset = random.randint(-200,200)
    yOffset = random.randint(-150,150)
    x = ((x2 - x1) * n) + x1 + xOffset
    y = ((y2 - y1) * n) + y1 + yOffset
